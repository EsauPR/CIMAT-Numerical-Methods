...
int main(int argc, char *argv[]) {
    ...
    NSMatrix eigen_vectors = matrix_eigen_jacobi_method_band(&matrix, 5);
    // matrixio_save_matrix(matrix, "eigen_values.mtx");
    matrixio_save_matrix(eigen_vectors, "eigen_vectors.mtx");

    NSVector lambdas;
    lambdas.items = matrixio_allocate_array_double(matrix.rows);
    lambdas.size = matrix.cols;

    for (int i = 0; i < matrix.rows; i++) {
        for (int j = 0; j < matrix.cols; j++) {
            lambdas.items[i] = matrix.items[i][j];
        }
    }

    matrixio_save_vector(lambdas, "eigen_values.mtx");
